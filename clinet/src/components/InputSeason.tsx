...
export default function InputSeason({category}:InputSeasonProp) {
    const navigate=useNavigate()
    const { id } = useParams<Params>();
      const { seasonId } = useParams<Params>();
      const curruntInput = years.find((year) => year.id === seasonId)?.input;
    
      const [season, setSeason] = useState<Year>({ input:curruntInput , id:seasonId  });
      ...
      const handleShowClick = () => {
        navigate(`/competitionProfile/${id}/${category}/${season.id}`)
