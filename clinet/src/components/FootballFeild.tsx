...
const FootballField: React.FC<PlayerProfile> = ({ playerMainPosition, secondPosition }) => {
  const mainPosition = `${playerMainPosition}`;
  const secondPositionStyle = secondPosition ? `${secondPosition}` : null;
  const playerStyle = positions[mainPosition] || { top: "50%", left: "50%" };
  const secondPlayerStyle = secondPositionStyle ? positions[secondPositionStyle] : null;
  ...
  return (
    <div className="relative w-[400px] h-[500px] bg-green-600 border-2 border-black">
      ...
      >
        <TbPlayFootball size={60} color="white" />
      </div>

      {secondPosition && secondPlayerStyle && (
        <div
          className="absolute"
          style={{
            top: secondPlayerStyle.top,
            left: secondPlayerStyle.left,
            transform: "translate(-50%, -50%)",
          }}
        >
          <TbPlayFootball size={30} color="white" />
        </div>
      )}
    </div>
  );
