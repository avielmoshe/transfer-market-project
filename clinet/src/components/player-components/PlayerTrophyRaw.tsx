  ...
  return (
    <div className="flex gap-[10px] p-[10px]">
      <div className="flex justify-center items-center">
        <img
          className="h-[100px]"
          src={competitionData.competition.trophy}
          alt=""
        />
      </div>
      ...
      <div>
        <div>
          {trophy.additionalData.map((data, index) => (
            <div
              key={index}
              className="flex items-center gap-[7px] text-[14px] p-[1px]"
            >
              <div>{data.seasonID}</div>
              <div>
                <img src={clubData.club.image} alt="" className="h-[30px]" />
              </div>
              <Link to={`/clubProfile/${clubId}/overview/2024`}>
                <div>{clubData.club.name}</div>
              </Link>
            </div>
          ))}
        </div>
